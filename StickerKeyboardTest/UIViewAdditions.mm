...
#define GRADIENT_LAYER_NAME @"gradientLayer"
///////////////////////////////////////////////////////////////////////////////////////////////////
...
-(void)resizeWidthToFitSubviews
{
    ...
    [self setFrame:CGRectMake(self.frame.origin.x, self.frame.origin.y, w + 5, self.frame.size.height)];
}
/*
...
-(void) addGradientBackgroundWithTopColor:(UIColor*)topColor andBottomColor:(UIColor*)bottomColor
{
    for (CALayer* layer in self.layer.sublayers) {
        if ([layer.name isEqualToString:GRADIENT_LAYER_NAME])
        {
            if (layer.frame.size.width != self.width) {
...
{
    [self fillSuperviewAndEnableAutoresizeMask];
...
- (NSArray *)constraintsForAttribute:(NSLayoutAttribute)attribute
{
    ...
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"firstAttribute = %d AND secondItem = NULL", attribute];
    filteredArray = [[self constraints] filteredArrayUsingPredicate:predicate];
    
    if (filteredArray.count == 0) {
        predicate = [NSPredicate predicateWithFormat:@"(secondAttribute = %d AND secondItem = %@)OR (firstAttribute = %d AND firstItem = %@)", attribute, self, attribute, self];
        filteredArray = [[self.superview constraints] filteredArrayUsingPredicate:predicate];
    }
